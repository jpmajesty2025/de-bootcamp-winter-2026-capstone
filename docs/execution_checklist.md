...
|---|---|---|---|---|---|---|---|
| A0 | Initialize UC Environment (Schema/Volumes) | P0 | DONE | You | 30m | None | `src/00_setup_env.py` runs successfully |
| A1a | Download CDC PLACES raw to Volume | P0 | DONE | You | 30m | A0 | `src/00a_download_cdc_places_bulk.py` lands CSV |
| A1b | CDC PLACES Bronze table ingestion | P0 | DONE | You | 1h | A1a | `src/01_ingest_cdc_places.py` creates Bronze Delta |
| A2a | Download Census ACS raw to Volume | P0 | DONE | You | 30m | A0 | `src/00b_download_census_acs.py` lands CSV |
...
| A3 | Silver conformance (FIPS/schema/date normalization) | P0 | IP | You | 2–3h | A2c | Conformed Silver tables queryable and keyed consistently |
